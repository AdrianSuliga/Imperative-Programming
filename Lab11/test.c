...
typedef struct Person {
	int age;
	char first_name[MAX_STR_LEN], last_name[MAX_STR_LEN];
} Person;
...
void resize(Vector *vector, size_t new_size) {
	...
	if (new_size > vector->size) {
		memset(
			vector->data + vector->size * vector->element_size,
			0,
...
void vector_test(
	Vector *vector, size_t block_size, size_t elem_size, int n,
	read_ptr read, cmp_ptr cmp, predicate_ptr predicate, print_ptr print
) {
...
int main(void) {
	...
	switch (to_do) {
		case 1:
			vector_test(
				&vector_int, 4, sizeof(int), n,
				read_int, int_cmp, is_even, print_int
			); break;
		case 2:
			vector_test(
				&vector_char, 2, sizeof(char), n,
				read_char, char_cmp, is_vowel, print_char
			); break;
		case 3:
			vector_test(
				&vector_person, 2, sizeof(Person), n,
				read_person, person_cmp, is_older_than_25, print_person
			); break;
		default:
