...
void mat_product(double A[][SIZE], double B[][SIZE], double AB[][SIZE], int m, int p, int n) {
	for (int i = 0; i < m; i++)
	{
		for (int j = 0; j < n; j++)
		{
			double num_to_write = 0.0;
			for (int k = 0; k < p; k++)
			{
				num_to_write += A[i][k] * B[k][j];
			}
			AB[i][j] = num_to_write;
		}
	}
...
void swap_rows(double A[][SIZE], int r1, int r2, int n)
{
	for (int i = 0; i < n; i++)
	{
		double buffor = A[r1][i];
		A[r1][i] = A[r2][i];
		A[r2][i] = buffor;
	}
}
void insert_max(double A[][SIZE], int n, int begin, int *det_m)
{
	int max_val = INT_MIN;
	int ind = 0;
	for (int i = begin; i < n; i++)
	{
		if (max_val < A[i][begin])
		{
			max_val = A[i][begin];
			ind = i;
		}
